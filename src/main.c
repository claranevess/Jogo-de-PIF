...
// Função para a transição entre mapas
        ...
        if (currentState == MENU) {
            ...
            for (int i = 0; i < MAX_COINS; i++) {
                if (!coins[i].collected) {
                    DrawCircle((int)coins[i].position.x, (int)coins[i].position.y, 10.0f, coins[i].color);
                }
            }

            DrawCircle((int)player.position.x, (int)player.position.y, 20.0f, GREEN);
